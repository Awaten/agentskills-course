...
| **中繼資料解讀不同** | `compatibility`、`allowed-tools` 等欄位並非每個平台都支援 | OpenCode 會用 `compatibility` 過濾技能，其他平台完全忽略 |
...
- **⚠️ 注意**：Codex 是 AWS 生態的一環，技能中的工具呼叫若涉及 AWS API（如 S3、Lambda），會有額外的 IAM 權限模型需要處理。你的技能可能需要提示使用者先設定 AWS 憑證。
...
- **載入方式**：使用 `skill` 工具載入，類似 Claude Code 的觸發機制。技能作者可以利用 `skill` 工具來載入其他技能，實現技能組合。
