...
1. **建立** 一個符合開放標準（Open Standard）的 Agent Skill 目錄結構（`agents/skills/<name>/`）與核心檔案 SKILL.md
...
此外，從零建置能讓你學到一個關鍵洞察：**Agent Skills 的本質不是程式，而是知識封裝（Knowledge Encapsulation）。** 當你親自走過每一個步驟，你會發現最困難的部分從來不是寫程式——而是如何將一個模糊的需求（「讓 Agent 學會擲骰子」）轉化為 Agent 可以精確執行的步驟序列。這種「需求→指令」的轉化能力，才是 Agent Skills 開發者最核心的技能。
